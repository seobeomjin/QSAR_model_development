...
import scipy as sc
import pandas as pd
from rdkit.Chem import PandasTools, Draw, AllChem, rdMolDescriptors
...
class CalculateContinuous:
    ...
    def get_significant_substructure_with_high_bioactivity(self,
        ...
        #if type(mols) != list:
        #    mols = [mols[i] for i in np.arange(0,len(mols))]
        nb_substructures_processed=0
        already_processed = []
        
        for akey in smi_dict.keys():#key is a chemblID
            mol = Chem.MolFromSmiles(smi_dict[akey])
            info={}
            fp = AllChem.GetMorganFingerprint(mol,max(self.radii_ext),bitInfo=info)
            substructures_sub_dict = substructure_dictionary.keys()
            
            infoitems = info.items()
            for substructure_id, atom_radius in infoitems:
                nb_substructures_processed += 1
                ...
                if substructure_id in substructures_sub_dict and atom_radius[0][1] in self.radii_ext and substructure_id not in already_processed:
                    ...
                    if filter_a and filter_b:
                        mask = np.in1d(mols_ids, substructure_dictionary[substructure_id])#.astype(int)
                        bio_substr = bioactivities[mask]
                        bio_wo_substr = bioactivities[np.logical_not(mask)]
                        
                        
                        #check if the current substructure dominantly located in the high activity group
                        nb_high_bio_in_substr = 0
                        for bioact in bio_substr : 
                            if bioact >= threshold_bioactivity:
                                nb_high_bio_in_substr += 1
                        #if not, disregard the current substructure
                        if nb_high_bio_in_substr <= len(bio_substr)//2:
                            continue
                        
                        
                        # check normality
                        if sc.stats.shapiro(bio_substr)[1] > 0.05 and sc.stats.shapiro(bio_wo_substr)[1] > 0.05:
                        ...
                        if p_value < threshold_pvalue: 
                            env = Chem.FindAtomEnvironmentOfRadiusN(mol,atom_radius[0][1],atom_radius[0][0])
                            #below codes are about important sumstructure features, but not used for now
                            #amap = {}
                            #submol=Chem.PathToSubmol(mol,env,atomMap=amap)
                            #m1 = mol
                            #m1.GetSubstructMatch(submol)

                            # extract_significant_substructure with high activity
                            if substructure_id in self.significant_substructure_dict.keys():
                                self.significant_substructure_dict[substructure_id].append(akey)
                            else : 
                                self.significant_substructure_dict.update({substructure_id:[akey]})
